...
function fill(template, port, token) {
  return template.replace('__PORT__', String(port)).replace('__TOKEN__', token);
}

export function generateAgent({ language, port, token }) {
  const p = safePort(port);
  const t = safeToken(token);
  const script = fill(language === 'python' ? PYTHON_AGENT : POWERSHELL_AGENT, p, t);
...
export function generateListener({ port, token }) {
  const p = safePort(port);
  const t = safeToken(token);
  const script = fill(PYTHON_LISTENER, p, t);
