...
function Login({ onLogin }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      onLogin({ port: j.port, host: j.host });
    ...
    <div style={S.center}>
      ...
      <form onSubmit={submit} style={S.card}>
        ...
        <div style={{ fontSize: 11, color: '#3a6a4a', marginBottom: 16 }}>
          أدخل عنوان المستمع والمنفذ اللذين شغّلتهما على جهازك (عنوان IP/نطاق جهازك الذي يستقبل الاتصال الخارجي + المنفذ).
        </div>
        <label style={S.lbl}>
          <span style={S.prompt}>{'>'}_ LISTENER_HOST</span>
          <input style={{ ...S.inp, direction: 'ltr' }} value={host} onChange={e => setHost(e.target.value)} placeholder="203.0.113.7 / my-pc.dyndns.org" required />
        </label>
        <label style={S.lbl}>
    ...
    <div style={S.wrap}>
      <header style={S.header}>
        <div style={{ fontSize: 14, color: G }}>
          <span style={S.online} />
          root@ops:~#<span style={{ color: '#3a6a4a' }}> {auth.host}:{auth.port}</span>
        </div>
        <button onClick={logout} style={{ ...S.btnSm, color: RED, borderColor: RED }}>EXIT</button>
      </header>

      <nav style={S.tabs}>
        <button type="button" onClick={() => setTab('terminal')} style={S.tab(tab === 'terminal')}>[1] TERMINAL</button>
        <button type="button" onClick={() => setTab('payload')} style={S.tab(tab === 'payload')}>[2] PAYLOAD</button>
      </nav>
      ...
      {tab === 'terminal' ? (
        <section>
          <div style={S.panel}>
            <div style={S.panelTitle}>$ command.execute --live</div>
    ...
    <div>
      <div style={S.panel}>
        ...
        <div style={{ fontSize: 11, color: '#3a6a4a', marginBottom: 14 }}>
          الوكيل المولَّد يحمل مضيفك ومنفذك ليعود إليك عبر النفق: {auth.host}:{auth.port}
        </div>
        <div style={S.fieldRow}>
          <label style={{ ...S.lbl, flex: 1 }}>
            <span style={S.prompt}>{'>'}_ INJECTED_HOST</span>
            <input style={{ ...S.inp, direction: 'ltr' }} value={auth.host} readOnly />
          </label>
          <label style={{ ...S.lbl, flex: 1 }}>
            <span style={S.prompt}>{'>'}_ INJECTED_PORT</span>
            <input style={{ ...S.inp, direction: 'ltr' }} value={auth.port} readOnly />
          </label>
          <label style={{ ...S.lbl, flex: 1 }}>
        ...
        <div style={{ display: 'flex', gap: 10, marginTop: 6 }}>
          ...
          <button onClick={genListener} style={S.btnSm} disabled={busy !== ''}>{busy === 'listener' ? 'BUILDING...' : '$ build listener (جهازك)'}</button>
        </div>
        {error && <div style={S.err}>[!] {error}</div>}
      </div>
      ...
      {listenerOut && (
        <div style={S.panel}>
          <div style={S.panelTitle}>$ {listenerOut.filename} — شغّله على جهازك أولاً</div>
      ...
      {agentOut && (
        <div style={S.panel}>
          <div style={S.panelTitle}>$ {agentOut.filename} — شغّله على الجهاز البعيد</div>
          <div style={{ display: 'flex', gap: 8, margin: '8px 0' }}>
