...
function Login({ onLogin }) {
  ...
  async function submit(e) {
    ...
    try {
      ...
      onLogin({ host: j.host, port: j.port });
  ...
  return (
    <div style={S.center}>
      ...
      <form onSubmit={submit} style={S.card}>
        ...
        <div style={{ fontSize: 11, color: '#3a6a4a', marginBottom: 16 }}>أدخل عنوان المستمع和个人ي الذي شغّلته على جهازك</div>
        <label style={S.lbl}>
          <span style={S.prompt}>{'>'}_ LISTENER_HOST</span>
          <input style={S.inp} value={host} onChange={e => setHost(e.target.value)} placeholder="my-pc.dyndns.org / 203.0.113.7" dir="ltr" required />
        </label>
        <label style={S.lbl}>
    ...
    <div>
      <div style={S.panel}>
        ...
        <div style={{ fontSize: 11, color: '#3a6a4a', marginBottom: 14 }}>
          البايلود سيحقن {auth.host}:{auth.port} تلقائياً — الوكيل سيعود نحو مستمعك (وليس نحو موقعنا).
        </div>
